...
N_LIB_PRIVATE N_NIMCALL(void, setLen__stdZenvvars_u123)(tySequence__4eQHGndY6XBYpFOH09apV8Q* s_p0, NI newlen_p1);
...
}
{
	NIM_BOOL T3_;
T3_ = ((NIM_BOOL) 0);
T3_ = eqeq___system_u8265(env_1, ((NCSTRING) NIM_NIL));
if ((!(T3_))) {
	goto LA4_;
}
...
T2_ = ((NIM_BOOL) 0);
T2_ = eqeq___system_u8265(T1_, ((NCSTRING) NIM_NIL));
result = (!(T2_));
}
	BeforeRet_: ;
		return result;
...
result.p = NIM_NIL;
T1_ = src_p0.len;
setLen__stdZenvvars_u123(((&result)), T1_);
colontmp_ = ((NI) 0);
{
	while (1) {
	NI T4_;
...
LA4_: ;
T6_ = src_p1.len;
setLen__stdZenvvars_u123(((&(*dest_p0))), T6_);
colontmp_ = ((NI) 0);
{
	while (1) {
	NI T9_;
