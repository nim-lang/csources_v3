...
}
{
	NIM_BOOL T3_;
T3_ = ((NIM_BOOL) 0);
T3_ = eqeq___system_u8302(env_1, ((NCSTRING) NIM_NIL));
if ((!(T3_))) {
	goto LA4_;
}
...
T7_ = ((NI32) 0);
T7_ = osLastError__stdZoserrors_u103();
if (NIM_UNLIKELY((*nimErr_))) {
	goto LA6_;
}
T8_.Field0 = key_p0;
...
raiseOSError__stdZoserrors_u100(T7_, colontmpD_);
if (NIM_UNLIKELY((*nimErr_))) {
	goto LA6_;
}
{
...
T2_ = ((NIM_BOOL) 0);
T2_ = eqeq___system_u8302(T1_, ((NCSTRING) NIM_NIL));
result = (!(T2_));
}
	BeforeRet_: ;
		return result;
...
raiseOSError__stdZoserrors_u100(T6_, key_p0);
if (NIM_UNLIKELY((*nimErr_))) {
	goto BeforeRet_;
}
}
